...
public class Parqueadero {
     ...
     * @return Respuesta 1.
     */
    public String metodo1() {ArrayList<Puesto> lugares_Ocupados = this.obtenerPuestosOcupados();

        String placa_Mayor = "No hay Vehiculos";
        int tiempo_Mayor = 0;
        for (Puesto puesto : lugares_Ocupados) {
            Carro carro = puesto.darCarro();
            int tiempo_Carro = carro.darTiempoEnParqueadero(this.horaActual);
            if (tiempo_Carro >= tiempo_Mayor) {
                tiempo_Mayor = tiempo_Carro;
                placa_Mayor = carro.darPlaca();
            }
        }
        return placa_Mayor;
    }
    private ArrayList<Puesto> obtenerPuestosOcupados() {
        ArrayList<Puesto> lugares_Ocupados = new ArrayList<Puesto>();
        for (int e = 0; e < TAMANO; e++) {
            if (this.estaOcupado(e)) {
                lugares_Ocupados.add(this.puestos[e]);
            }
        }
    ...
    public String metodo2() {
        ArrayList<Puesto> lugares_Ocupados = this.obtenerPuestosOcupados();
        if (lugares_Ocupados.size() == TAMANO) {
